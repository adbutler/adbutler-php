...
class CURLClientTest extends TestUtils
{
    ...
    public function testEncodedURLs()
    {
        ...
        CURLClient::init(array('api_key' => $this->apiKey));
        $response = $this->invokeMethod('AdButler\CURLClient', 'constructRequestURL',
            array("$this->baseURL/$this->version/stats", 123, $queryParams, array()));
        $this->assertSame('http://api.adbutler.com.baig.dev/v1/stats/123?type=publisher&from=2016-09-01T00%3A00%3A00%2B00%3A00&to=2016-10-15T16%3A00%3A00%2B00%3A00&period=year',
            $response);
    }
}